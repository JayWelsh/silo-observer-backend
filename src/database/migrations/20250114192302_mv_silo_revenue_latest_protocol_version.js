...
exports.up = (knex) => knex.schema.raw(`
  ...
  WITH latest_timestamps AS (
    ...
    GROUP BY asset_address, network, protocol_version, silo_address
  )
  SELECT DISTINCT
    srs.asset_address,
    srs.network,
    srs.protocol_version,
    srs.silo_address,
    srs.amount_pending,
  ...
  JOIN ${SILO_REVENUE_SNAPSHOT_TABLE} srs
    ...
    AND srs.protocol_version = lt.protocol_version
  ...
  -- Modified indexes to include protocol_version in the unique constraint
  CREATE UNIQUE INDEX latest_snapshots_composite_idx 
    ON ${LATEST_SILO_REVENUE_SNAPSHOT_MATERIALIZED_VIEW} (asset_address, network, protocol_version, silo_address, timestamp);
  ...
  CREATE INDEX latest_snapshots_network_protocol_idx 
    ON ${LATEST_SILO_REVENUE_SNAPSHOT_MATERIALIZED_VIEW} (network, protocol_version);
`);
