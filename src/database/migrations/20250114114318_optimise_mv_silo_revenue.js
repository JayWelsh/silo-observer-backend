...
time_network_series AS (
  SELECT 
    ...
    n.network
  FROM time_series ts
  CROSS JOIN (SELECT DISTINCT network FROM ${SILO_REVENUE_SNAPSHOT_TABLE}) n
),
snapshot_values AS (
  SELECT 
    ...
    ROW_NUMBER() OVER (
      PARTITION BY network, date_trunc('hour', timestamp)
      ORDER BY timestamp DESC
    ) as rn
  FROM ${SILO_REVENUE_SNAPSHOT_TABLE}
  GROUP BY date_trunc('hour', timestamp), network, timestamp
),
latest_values AS (
  SELECT 
    tns.hour_timestamp,
