...
exports.up = (knex) => knex.schema.alterTable(RATE_HOURLY_TABLE, table => {
  ...
  await knex(RATE_HOURLY_TABLE).where({ network: "ethereum" }).update('deployment_id', 'ethereum-original');
  return knex(RATE_HOURLY_TABLE).where({ network: "arbitrum" }).update('deployment_id', 'arbitrum-original');
}).then(() => knex.schema.alterTable(RATE_HOURLY_TABLE, table => {
