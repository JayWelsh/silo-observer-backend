...
exports.up = (knex) => knex.schema.alterTable(BORROWED_MINUTELY_TABLE, table => {
  ...
  await knex(BORROWED_MINUTELY_TABLE).where({ network: "ethereum" }).update('deployment_id', 'ethereum-original');
  return knex(BORROWED_MINUTELY_TABLE).where({ network: "arbitrum" }).update('deployment_id', 'arbitrum-original');
}).then(() => knex.schema.alterTable(BORROWED_MINUTELY_TABLE, table => {
