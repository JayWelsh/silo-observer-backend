...
export const getAllSiloBorrowEventsSinceBlock = async (
  ...
  let eventIndexBlockTrackerRecord = await EventIndexerBlockTrackerRepository.findByColumn("event_name", "Borrow");
  ...
  for(let siloAddress of siloAddresses) {
    ...
    let events = await eventIndexer(siloContract, borrowEventFilter, lastestBlock, fromBlock, toBlock, blockRange, `${siloAddress} (Borrow) - silo ${siloProgress} of ${siloAddresses.length}`);
    if(events) {
