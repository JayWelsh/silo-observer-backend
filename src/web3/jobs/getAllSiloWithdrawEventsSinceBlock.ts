...
export const getAllSiloWithdrawEventsSinceBlock = async (
  ...
  if(blockRange > 1) {
    ...
    for(let siloAddress of siloAddresses) {
      ...
      let events = await eventIndexer(siloContract, withdrawEventFilter, lastestBlock, fromBlock, toBlock, blockRange, network, `${siloAddress} - isSanityCheck: ${isSanityCheck} - (Withdraw) - silo ${siloProgress} of ${siloAddresses.length}`);
      if(events) {
