...
export const periodicContractEventTracker = async (useTimestampUnix: number, startTime: number) => {
  ...
  try {
    ...
    let siloAddresses = await getAllSiloAddresses();

    let [...eventBatches] = await Promise.all([
      getAllSiloBorrowEventsSinceBlock(siloAddresses, latestBlockNumber),
      getAllSiloDepositEventsSinceBlock(siloAddresses, latestBlockNumber),
      getAllSiloRepayEventsSinceBlock(siloAddresses, latestBlockNumber),
      getAllSiloWithdrawEventsSinceBlock(siloAddresses, latestBlockNumber),
    ])
    ...
    for(let blockNumber of allBlockNumbers) {
      let currentRecord = await BlockMetadataRepository.findByColumn('block_number', blockNumber);
      if(!currentRecord) {
    ...
    if(unfetchedBlockNumbers && unfetchedBlockNumbers.length > 0) {
      let blockData = await getBlocks(unfetchedBlockNumbers);
    ...
    console.log(`Periodic contract event tracker successful, exec time: ${new Date().getTime() - startTime}ms`)

  } catch (e) {
    console.error(`Error encountered in periodicContractEventTracker at ${useTimestampPostgres}, exec time: ${new Date().getTime() - startTime}ms`)
  }
